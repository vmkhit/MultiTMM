...
for i = 1:nth
    ...
    file1 = readdlm(join([path_ell, "190327_Chip-Inaki-Laura-S84-12ML_an15_Si_", string(theta),".pae"]))
    file2 = readdlm(join([path_ell, "190327_Chip-Inaki-Laura-S84-12ML_an15_Ag_pos1_", string(theta),".pae"]))
    file3 = readdlm(join([path_ell, "190327_Chip-Inaki-Laura-S84-12ML_an15_Ag_pos2_", string(theta),".pae"]))
...
begin
    ...
    for i = 1:nth
        ...
        plot(DATA2[:, 1, i], DATA2[:, 2, i], linestyle="-", linewidth=2, color = "orange")
        #plot(DATA3[:, 1, i], DATA3[:, 2, i], linestyle="-", linewidth=2, color = "green")
        #plot(DATA1[:, 1, i], DATA1[:, 2, i], linestyle="--", linewidth=2, color = "blue")
    ...
    plot([],[], linestyle="-", linewidth=2, color = "orange", label = "S84, pos.1 /190327")
    #plot([],[], linestyle="-", linewidth=2, color = "green", label = "S84, pos.2 / 190327")
    #plot([],[], linestyle="--", linewidth=2, color = "blue", label = "Si /190327")
    ...
    for i = 1:nth
        ...
        plot(DATA2[:, 1, i], DATA2[:, 3, i], linestyle="-", linewidth=2, color = "orange")
        #plot(DATA3[:, 1, i], DATA3[:, 3, i], linestyle="-", linewidth=2, color = "green")
        #plot(DATA1[:, 1, i], DATA1[:, 3, i], linestyle="--", linewidth=2, color = "blue")
    ...
    legend(frameon=false, fontsize = 14)
    tight_layout()
    savefig("C:\\Users\\vmkhi\\Documents\\Projects\\Ribbons\\Chip - Laura - Andrew - S84\\Ellipsometry Pictures\\S84__12ML_epx_Fit.png", transparent = true)
end
