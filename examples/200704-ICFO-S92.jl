...
DATA2 = Array{Float64}(undef, 1103-79, 5, nth, 8)

path_ell = "C:\\Users\\vmkhitaryan\\Documents\\Projects\\Ag Ribbons\\Chip-Andrew-S92\\Ellipsometry\\"
for i = 1:nth
    theta = round(Int, th[i])

    file1 = readdlm(join([path_ell, "2020-06-04-Chip-Andrew-S92_an25_MS_Ag12ML_", string(theta),".pae"]))
    file2 = readdlm(join([path_ell, "2020-06-04-Chip-Andrew-S92_an25_MS_Ag16ML_", string(theta),".pae"]))
    file3 = readdlm(join([path_ell, "2020-06-04-Chip-Andrew-S92_an25_MS_Ag16ML_again_", string(theta),".pae"]))
    file4 = readdlm(join([path_ell, "2020-06-04-Chip-Andrew-S92_an25_MS_Si_", string(theta),".pae"]))
    ...
    for j = 1:8
        jj = j-1
        filex = readdlm(join([path_ell, "2020-06-29-Chip-S92-inc60to_an15_SiStart_$(jj)mm_", string(theta),".pae"]))
...
begin
    ...
    savefig(join([path_ell,"/Pictures/S92.png"]), transparent = false)
