...
DS87_1 = Array{Float64}(undef, 1103-79, 5, nth)
DS87_2 = Array{Float64}(undef, 1103-79, 5, nth)
DS87_3 = Array{Float64}(undef, 1103-79, 5, nth)

path_S87 = "C:\\Users\\vmkhi\\Documents\\Projects\\Ag Ribbons (Plasmons)\\Chip-Andrew-Laura-S87\\Ellipsometry\\"
for i = 1:nth
    theta = round(Int, th[i])
    fS87_1 = readdlm(join([path_S87, "190625_Chip-Inaki-Laura-S87-15ML_an15_Si_", string(theta),".pae"]))
    fS87_2 = readdlm(join([path_S87, "190625_Chip-Inaki-Laura-S87-15ML_an15_Ag_pos1_", string(theta),".pae"]))
    fS87_3 = readdlm(join([path_S87, "190625_Chip-Inaki-Laura-S87-15ML_an15_Ag_pos2_", string(theta),".pae"]))

    DS87_1[:, :, i] = fS87_1[79:end-1, 1:5]
    DS87_2[:, :, i] = fS87_2[79:end-1, 1:5]
    DS87_3[:, :, i] = fS87_3[79:end-1, 1:5]
end
...
begin
    ...
    for i = 1:nth
        ...
        plot(DS87_1[:, 1, i], DS87_1[:, 2, i], linestyle="-", linewidth=2, color = "blue")
        plot(DS87_2[:, 1, i], DS87_2[:, 2, i], linestyle="-", linewidth=2, color = "orange")
        plot(DS87_3[:, 1, i], DS87_3[:, 2, i], linestyle="-", linewidth=2, color = "green")
    ...
    plot([],[], linestyle="-", linewidth=2, color = "blue", label = "S87, Si")
    plot([],[], linestyle="-", linewidth=2, color = "orange", label = "S87, Ag pos.1")
    plot([],[], linestyle="-", linewidth=2, color = "green", label = "S87, Ag pos.2")
    ...
    for i = 1:nth
        ...
        # end
        #
        plot(DS87_1[:, 1, i], DS87_1[:, 3, i], linestyle="-", linewidth=2, color = "blue")
        plot(DS87_2[:, 1, i], DS87_2[:, 3, i], linestyle="-", linewidth=2, color = "orange")
        plot(DS87_3[:, 1, i], DS87_3[:, 3, i], linestyle="-", linewidth=2, color = "green")
    ...
    legend(frameon=false, fontsize = 14)
    tight_layout()
    suptitle("Ag 15ML + 1nm SiO2", fontsize = 16)
    savefig("C:\\Users\\vmkhi\\Documents\\Projects\\Ag Ribbons (Plasmons)\\Chip-Andrew-Laura-S87\\Ellipsometry\\Pictures\\S87_exp_only.png", transparent = true)
end
