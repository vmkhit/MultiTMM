...
DATA6 = Array{Float64}(undef, 1074-79, 5, nth)
DATA7 = Array{Float64}(undef, 1074-79, 5, nth)
...
for i = 1:nth
    ...
    file1 = readdlm(join([path_ell, "190226_Chip-Inaki-Laura-S82-12ML_an15_Ag_pos1_", string(theta),".pae"]))
    file2 = readdlm(join([path_ell, "190226_Chip-Inaki-Laura-S82-12ML_an15_Ag_pos2_", string(theta),".pae"]))
    file3 = readdlm(join([path_ell, "190226_Chip-Inaki-Laura-S82-12ML_an15_Si_", string(theta),".pae"]))

    file4 = readdlm(join([path_ell, "190227_Chip-Inaki-Laura-S82-12ML_an15_AfterLiftOff_Ag_pos1_", string(theta),".pae"]))
    file5 = readdlm(join([path_ell, "190227_Chip-Inaki-Laura-S82-12ML_an15_AfterLiftOff_Ag_pos2_", string(theta),".pae"]))

    file6 = readdlm(join([path_ell, "20190301_chip-Zaka-S82_Plane_CDD_inc60o75_an15_MS_Ag_pos1_", string(theta),".pae"]))
    file7 = readdlm(join([path_ell, "20190301_chip-Zaka-S82_Plane_CDD_inc60o75_an15_MS_Ag_pos2.pae_", string(theta),".pae"]))
...
begin
    ...
    for i = 1:nth
        ...
        #plot(DATA5[:, 1, i], DATA5[:, 2, i], linestyle="-.", linewidth=2, color = "black")

        #plot(DATA6[:, 1, i], DATA6[:, 2, i], linestyle="--", linewidth=2, color = "purple")
        #plot(DATA7[:, 1, i], DATA7[:, 2, i], linestyle="--", linewidth=2, color = "cyan")
        #=
        for (l, bl) = enumerate(blist)
    ...
    plot([],[], linestyle="-", linewidth=2, color = "orange", label = "Ag 12ML, pos.1 /190226")
    #plot([],[], linestyle="-", linewidth=2, color = "green", label = "Ag 12ML, pos.2 / 190226")
    #plot([],[], linestyle="--", linewidth=2, color = "blue", label = "Si")

    #plot([],[], linestyle="-.", linewidth=2, color = "red", label = "Ag 12ML, pos.1 /190227")
    #plot([],[], linestyle="-.", linewidth=2, color = "black", label = "Ag 12ML, pos.2 /190227")

    #plot([],[], linestyle="--", linewidth=2, color = "purple", label = "Ag 12ML, pos.1 /190231")
    #plot([],[], linestyle="--", linewidth=2, color = "cyan", label = "Ag 12ML, pos.2 /190231")
    ...
    for i = 1:nth
        ...
        #plot(DATA5[:, 1, i], DATA5[:, 3, i], linestyle="-.", linewidth=2, color = "black")

        #plot(DATA6[:, 1, i], DATA6[:, 3, i], linestyle="--", linewidth=2, color = "purple")
        #plot(DATA7[:, 1, i], DATA7[:, 3, i], linestyle="--", linewidth=2, color = "cyan")
    ...
    legend(frameon=false, fontsize = 14)
    tight_layout()
    savefig("C:\\Users\\vmkhi\\Documents\\Projects\\Ribbons\\Chip-Laura-S82\\Elliposmetry Pictures\\FIT_190226_Ellipsometry_12ML_S82_gam10_30.png", transparent = true)
end
